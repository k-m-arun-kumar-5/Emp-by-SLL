...
#define TRACE_ON         1
#define TRACE_OFF        0
#define MEGA_DATA_PRINT 03  /* display mega_data info for debug */
...
struct employee_list                      /* data struct */
{
  ...
  struct employee_list *phoneforeptr;     /* emp_phone front ptr */
  struct employee_list *phonebackptr;     /* emp_phone back ptr */
};
...
struct megadata                            /* link control */
{
  ...
  emp_record *phoneheadptr;                /*  contains emp_phone head's node */
  emp_record *phonetailptr;                /*  contains emp_phone tail's node */
...
int trace_flag = TRACE_ON;                 /* global trace eecution for debugging */
...
extern int Delete_EmpNode(int target_mode, int nodepos);
extern int Get_DelEmpData(int del_mode);
extern int Print_EmpList( int target_mode);
extern int Search_EmpNode(int target_mode, void *dataptr, void *target, int phone_size);
...
int Initialize_MegaEmpList(void)
{
   ...
   mega_head.phoneheadptr = NULL;
   mega_head.phonetailptr = NULL;
...
int Get_EmpData(void)
{
   ...
   printf("\n Enter Employee's ID : ");
   scanf("%ld", &emp_id);
   nodepos = Search_EmpNode(EMPID_TARGET, &retrieve_empdata, &emp_id, INVALID_DATA);
   ...
   if(!Add_EmpNode(EMPID_TARGET, get_empdata,modify_flag ))
   	   return FALSE;
   if(trace_flag)
      printf("\n TRACE[5.1]: Taken Emp Record: id %ld, name: %s, phone: %s", get_empdata.emp_id,get_empdata.emp_name, get_empdata.emp_phone );
   return TRUE;
}
...
/* ==================================================================
   ...
   fUNC_ID       : 6
=================================================================== */
int Search_EmpNode(int target_mode, void *dataptr, void *target, int max_size)
{
    int nodepos = 1;            /* curptr node position from its head */
    ...
    switch(target_mode)
    {
	   case EMPID_TARGET:
		  ...
		  while( mega_head.curptr != NULL && target_empid > mega_head.curptr->emp_data.emp_id)
          {
		  ...
		  if(trace_flag)
	         printf("\n TRACE[6.2]: Id: %ld not found",target_empid);
		  nodepos = FALSE;
		  break;
	   case DEL_WHOLE_LIST:
	      Access_Curptr(EMPID_ACCESS);
    ...
    if(retrieve_flag == TRUE)
    {
        /* data retrieve for target which is unique & ordered */
	     *(struct employee *)dataptr = mega_head.curptr->emp_data;
	/*   if(trace_flag)
   ...
   if(mega_head.access_curptr != FREE_ACCESS)
   {
    ...
    switch(target_mode)
    {
		 ...
		 /* printf("\n Display Employee's record based on EMP_ID's list:");
		  ...
		  while(curptr)
		  {
		      ...
		      printf("\n %02d Node Data:: ID: %10ld, name: %10s, phone: %10s",\
		         i,curptr->emp_data.emp_id,curptr->emp_data.emp_name, curptr->emp_data.emp_phone,curptr);
		      printf("\n %02d: Addr Node: %#X, ID Front: %#X, ID Back: %#X, Ph Front: %#X, Ph Back: %#X ",\
		        i, curptr, curptr->idforeptr,curptr->idbackptr,curptr->phoneforeptr,curptr->phonebackptr );
        ...
        case MEGA_DATA_PRINT:
           ...
           printf("\n idheadptr: %#X, idtailptr: %#X, phoneheadptr: %#X, phonetailptr: %#X", \
            mega_head.idheadptr, mega_head.idtailptr, mega_head.phoneheadptr, mega_head.phonetailptr);
          /* printf("\n ================================================="); */
          break;
        default:
...
/* ==================================================================
   ...
   fUNC_ID       : 10
=================================================================== */
int Delete_EmpData(int target_mode, void *from_del, void *to_del, int ph_size, struct employee *get_delnodedata)
{
	int nodepos =0, to_nodepos =0, from_node_pos;
    int ret_state = FALSE;
    int node_count = 0;
	...
	{
       case EMPID_TARGET:
	     nodepos = Search_EmpNode(target_mode, get_delnodedata, from_del, INVALID_DATA);
         if(nodepos == FALSE)
         {
	     ...
	     if(trace_flag)
           printf("\n TRACE[10.1]: [%d] Del node pos, Emp ID: %ld, name: %s, phone: %s, Addr: %#x", \
             nodepos,get_delnodedata->emp_id, get_delnodedata->emp_name, get_delnodedata->emp_phone,mega_head.curptr);
         ret_state = TRUE;
         break;
       case DEL_WHOLE_LIST:
	     nodepos = Search_EmpNode(target_mode,get_delnodedata , NULL, INVALID_DATA );
         if(!nodepos)
         {
			 printf("\n ERR[10.3]: Try to Destroy already non existing List");
			 break;
	     }
	     /* return number of nodes to be deleted */
	     ret_state = nodepos;
         break;
      /* case EMPID_RANGE:
         from_iddel = *(long int *) from_del;
         to_iddel = *(long int *) to_del;
         from_nodepos = Search_EmpNode(target_mode,get_delnodedata, from_del, INVALID_DATA );
         if(!nodepos)
         {
    ...
    if(ret_state == FALSE || !Delete_EmpNode(target_mode,nodepos))
...
/* ==================================================================
   ...
   fUNC_ID       : 11
=================================================================== */
int Delete_EmpNode(int target_mode, int nodepos)
{
	int ret_state = FALSE;
...
int Get_DelEmpData(int del_mode)
{
    ...
    {
		case EMPID_TARGET:
          printf("\n Enter Emp ID's range(from & to) to be deleted: ");
          scanf("%ld %ld", &from_iddel, &to_iddel);
          /* here: add codes to if u want to validate is numeric one, etc */
          /* to delete specfic empid  */
           if(from_iddel == to_iddel)
           {
		      if(Delete_EmpData(del_mode, &from_iddel, NULL,INVALID_DATA, get_delempdata ))
			  {
		   ...
		   /* if(!Delete_EmpData(EMPID_RANGE, &from_iddel, &to_iddel,INVALID_DATA, get_delempdata ))
		   	  break;
		   ret_state = TRUE; */
		   printf("\n ERR[12.2]: Support only single empid based deletion");
		   break;
		 case DEL_WHOLE_LIST:
           node_num = Delete_EmpData(del_mode, NULL, NULL,INVALID_DATA, get_delempdata );
           if(node_num)
           {
...
int Retrieve_EmpData(int retrieve_mode)
{
	int ret_state = FALSE;
    int nodepos;
    struct employee retrieve_empdata[OPER_SIZE];
    ...
    switch(retrieve_mode)
    {
		case EMPID_TARGET:
	      ...
	      nodepos = Search_EmpNode(retrieve_mode, retrieve_empdata, &retrieve_empid, INVALID_DATA);
          if( nodepos != FALSE )
          {
            /* retrieve_empdata contains emp record for existing empid */
		     printf("\n Emp Record: ID: %ld, name: %s, phone: %s",retrieve_empdata[count_node].emp_id, retrieve_empdata[count_node].emp_name, retrieve_empdata[count_node].emp_phone );
	         ret_state = TRUE;
	      }
	      else
		...
		case EMPNAME_TARGET:
		  ...
		  scanf(" %[^\n]",retrieve_empname);
	      nodepos = Search_EmpNode(retrieve_mode, retrieve_empdata, retrieve_empname, sizeof(retrieve_empname));
          if( nodepos != FALSE )
          {
    		  while(count_node < nodepos)
			  {
				  printf("\n Matched Emp name: %s record, empid: %ld, empphone: %s",retrieve_empdata[count_node].emp_name,retrieve_empdata[count_node].emp_id,retrieve_empdata[count_node].emp_phone);
				  ++count_node;
		      }
              ret_state = TRUE;
	      }
